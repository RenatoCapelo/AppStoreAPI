...
namespace AppStoreAPI.Services.ApkParser
{
    ...
    public class ApkParser
    {
        ...
        public static ApkInfo Parse(string extensionsPath, string apkFilePath)
        {
            if (!File.Exists(apkFilePath))
                throw new FileNotFoundException(apkFilePath + " file not found!");
            string output = ConsoleExecutor.Execute(@$"{extensionsPath}\aapt.exe", "dump",
                "badging", "\"" + apkFilePath + "\"");
            ...
            SetIcons(apkInfo, apkOut, apkFilePath,extensionsPath);
            return apkInfo;
        }
        ...
        private static void SetIcons(ApkInfo apkInfo, string[] apkOut, string apkFilePath,string extensionsPath)
        {
            ...
            foreach (var icon in icons)
            {
                ...
                ConsoleExecutor.Execute(@$"{extensionsPath}\unzip.exe", "-j", "-o", "\"" + apkFilePath + "\"",
                    icon, "-d", "\"" + dir + "\"");
